  ...
  if (req.body.store == '1') {
    Room.findById(req.params.roomId)
      .then(room => {
        ...
        fetchedRoom = room;
        const currentPeople = new People( { data: socketPeopleCount , time: Date(req.body.datetime)} );
...
exports.putPeople = (req, res) =>  {
  ...
  if (!fs.existsSync(path)) {
    ...
    socket.emit("people", {people: req.body.data, roomId: req.params.roomId, store: false});
  }
  ...
  while ((read = fs.readSync(t1, buffer, 0, bufferSize, null)) !== 0) {
    ...
    while ((idx = leftOver.indexOf("\n", idxStart)) !== -1) {
      ...
      if (roomId == req.params.roomId) {
        ...
        // Read the file 
        fs.readFile("people.txt", "utf-8", function (err, data) {
          let newPeopleCount = data.replace(re, (roomId + ":" + String(parseInt(req.body.data))));
          socketPeopleCount = "" + String(parseInt(req.body.data));
...
exports.getRealTimePeople = (req, res) => {
  let found = false;
  let peopleCount = 0;
  ...
  if (!fs.existsSync(path)) {
    ...
    // Here add the room id and the number of people, separate by ':'
    fs.appendFileSync("people.txt", req.params.roomId + ":" + 0 + "\n");
  }
  ...
  while ((read = fs.readSync(t1, buffer, 0, bufferSize, null)) !== 0) {
    ...
    while ((idx = leftOver.indexOf("\n", idxStart)) !== -1) {
      ...
      // Check whether the line's room id whether it is identical to the http request room id
      if (roomId == req.params.roomId) {
        peopleCount = line.slice(semicolon + 1, line.length).trim();
        found = true;
      }
    }
