				...
				if(result.rooms[room]._id == roomId){
					...
					for(var people in result.rooms[room].people){
                        ...
                        for(var i = 0; i < people.length; i ++){
							if(date.getDay() == dateToday){
								tp[i] += result.rooms[room].people[people].data;
                                timetoday[i] += result.rooms[room].people[people].time;
...
function showTodayTemperatureChart(){
	var url_string = window.location.href;
	var url = new URL(url_string);
	var pathname = url.pathname;
	var split = pathname.split("/"); 
					...
					for(var temp in result.rooms[room].temperature){
						...
						for(var i = 0; i < temp.length; i++){
							if(date.getDay() == dateToday)
                                ...
                                timetoday[i] += result.rooms[room].temperature[temp].time;
								counter[i] = counter[i] + 1;
						}
...
function showTodayHumidityChart(){
	var url_string = window.location.href;
	var url = new URL(url_string);
	var pathname = url.pathname;
	var split = pathname.split("/"); 
...
function showTodayChart() {
	showTodayPeopleChart();
	showTodayTemperatureChart();
	showTodayHumidityChart();
};


function showAllTimePeopleChart(){
	var url_string = window.location.href;
	var url = new URL(url_string);
	var pathname = url.pathname;
	var split = pathname.split("/"); 
			...
			for(var room in result.rooms){	
				if(result.rooms[room]._id == roomId){
						...
						for(var people in result.rooms[room].people){
									...
									peopleNo[people] = result.rooms[room].people[people].data;
									timetoday[people] = new Date(result.rooms[room].people[people].time);
									timetoday[people] = timetoday[people].toLocaleString();
						}				
				}
			}
...
function showAllTimeTemperatureChart(){
	var url_string = window.location.href;
	var url = new URL(url_string);
	var pathname = url.pathname;
	var split = pathname.split("/"); 
			...
			for(var room in result.rooms){	
				if(result.rooms[room]._id == roomId){
					...
					for(var temp in result.rooms[room].temperature){
								...
								tempNo[temp] = result.rooms[room].temperature[temp].data;
								timetoday[temp] = new Date(result.rooms[room].temperature[temp].time);
								timetoday[temp] = timetoday[temp].toLocaleString();
					}
				}
			}
