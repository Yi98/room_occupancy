...
socket.on("people", function (msg) {
	...
	for (let i = 0; i < roomCards.length; i++) {
		...
		if (roomId[i].innerHTML == msg.roomId) {
			let roomId = roomCards[i].getElementsByClassName("roomId")[i];
			let maxCapacity = document.getElementsByClassName('maxCapacity')[i];
			let division = Math.round(msg.people / maxCapacity) * 10;
			...
			console.log(maxCapacity - division);

			document.getElementsByClassName('status-indicator-outer')[0].style.width = (maxCapacity - division) + '%';
		}
	}
